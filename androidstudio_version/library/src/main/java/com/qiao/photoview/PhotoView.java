...
public class PhotoView extends ImageView {
    ...
    private void initBase() {
        ...
        if (mImageOption != null && System.currentTimeMillis() - mInfoTime < MAX_ANIM_FROM_WAITE) {
            animaFrom(mImageOption);
        }
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    public ImageOption getInfo() {
        ...
        return new ImageOption(rect, local, mImgRect, mWidgetRect, mScale, mDegrees, mScaleType);
    }
     ...
     */
    public void animaFrom(ImageOption imageOption) {

        if (isInit) {
            reset();

            ImageOption mine = getInfo();

            float scaleX = imageOption.mImgRect.width() / mine.mImgRect.width();
            float scaleY = imageOption.mImgRect.height() / mine.mImgRect.height();
            float scale = scaleX < scaleY ? scaleX : scaleY;

            float ocx = imageOption.mRect.left + imageOption.mRect.width() / 2;
            float ocy = imageOption.mRect.top + imageOption.mRect.height() / 2;

            mAnimaMatrix.reset();
            mAnimaMatrix.postTranslate(-mBaseRect.left, -mBaseRect.top);
            mAnimaMatrix.postTranslate(ocx - mBaseRect.width() / 2, ocy - mBaseRect.height() / 2);
            mAnimaMatrix.postScale(scale, scale, ocx, ocy);
            mAnimaMatrix.postRotate(imageOption.mDegrees, ocx, ocy);
            executeTranslate();
            ...
            mTranslate.withRotate((int) imageOption.mDegrees, 0);

            if (imageOption.mWidgetRect.width() < imageOption.mImgRect.width() || imageOption.mWidgetRect.height() < imageOption.mImgRect.height()) {
                float clipX = imageOption.mWidgetRect.width() / imageOption.mImgRect.width();
                float clipY = imageOption.mWidgetRect.height() / imageOption.mImgRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;

                ClipCalculate c = imageOption.mScaleType == ScaleType.FIT_START ? new START() : imageOption.mScaleType == ScaleType.FIT_END ? new END() : new OTHER();
    ...
    public void animaTo(ImageOption imageOption, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
            ...
            float tcx = imageOption.mRect.left + imageOption.mRect.width() / 2;
            float tcy = imageOption.mRect.top + imageOption.mRect.height() / 2;
            ...
            // 缩放
            float scaleX = imageOption.mImgRect.width() / mBaseRect.width();
            float scaleY = imageOption.mImgRect.height() / mBaseRect.height();
            float scale = scaleX > scaleY ? scaleX : scaleY;
            ...
            mTranslate.withRotate((int) mDegrees, (int) imageOption.mDegrees, ANIMA_DURING * 2 / 3);

            if (imageOption.mWidgetRect.width() < imageOption.mRect.width() || imageOption.mWidgetRect.height() < imageOption.mRect.height()) {
                float clipX = imageOption.mWidgetRect.width() / imageOption.mRect.width();
                float clipY = imageOption.mWidgetRect.height() / imageOption.mRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;

                final float cx = clipX;
                final float cy = clipY;
                final ClipCalculate c = imageOption.mScaleType == ScaleType.FIT_START ? new START() : imageOption.mScaleType == ScaleType.FIT_END ? new END() : new OTHER();
